...
function styles() {
    return src('#src/scss/style.scss')     // файл(ы) с которым мы работаем
        .pipe(scss({
            outputStyle: 'expanded'    // compressed/expanded(без сжатия)
        }))
...
function scripts() {
    ...
    .pipe(babel({   //babel перед конкат. и миниф. 
        // не забываем создавать файл .babelrc с таким содержимым
...
function images() {
    return src('#src/img/**/*')
        ...
        // .pipe(webp())   //формат webp если нужно
        .pipe(dest('dist/img'))
}
